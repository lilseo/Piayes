...
void SynthAudioSource::getNextAudioBlock (const AudioSourceChannelInfo& bufferToFill) {
    ...
    if (MainContentComponent::whatChordActive())
        keyboardState.processNextMidiBuffer (incomingMidi, 0, bufferToFill.numSamples, true, MainContentComponent::whatChordActive());
    else
...
// Returns a value based on what, if any, chord feature is active
// 0 = no chord feature active, 1 = major
int MainContentComponent::whatChordActive() {
    if (setChordMajor)
        return 1;
    else
        return 0;
...
void MainContentComponent::handleNoteOff (MidiKeyboardState*, int midiChannel, int midiNoteNumber, float /*velocity*/) {
    ...
    if (setChordMajor) {
        MidiMessage m3 (MidiMessage::noteOff (midiChannel, m.getNoteNumber() + 4));
    }
