...
private:
    ...
    std::vector<float> handleNoteOn (MidiKeyboardState*, int midiChannel, int midiNoteNumber, float velocity) override
    {
        std::vector<float> noteTimes;
        std::cout << "midiNoteNumber: " << midiNoteNumber << "velocity: " << velocity << std::endl;
        if (! isAddingFromMidiInput)
        {
            MidiMessage m (MidiMessage::noteOn (midiChannel, midiNoteNumber, velocity));
            m.setTimeStamp (Time::getMillisecondCounterHiRes() * 0.001);
            postMessageToList (m);
            // Change note off timestamp to 2 seconds after note on
            MidiMessage m2 (MidiMessage::noteOff (midiChannel, midiNoteNumber, velocity));
            m2.setTimeStamp (m.getTimeStamp() + 2);
            cout << "MidiDemo note off timestamp: " << m.getTimeStamp() << endl;
            postMessageToList (m2);
            // add note on and off times to noteTimes
            noteTimes.push_back(m.getTimeStamp());
            noteTimes.push_back(m2.getTimeStamp()); 
        }
        return noteTimes; 
    }
    ...
    {/*
        if (! isAddingFromMidiInput)
        {
            ...
            m.setTimeStamp (Time::getMillisecondCounterHiRes() * 0.001);
            cout << "note off timestamp: " << m.getTimeStamp() << endl;
            postMessageToList (m);
        }
      */
    }
